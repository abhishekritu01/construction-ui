...
import { Button } from './Button';
import { ArrowRight, Play, Award, Users, Building, HardHat } from "lucide-react";
import { motion, useInView, useAnimation, stagger, animate, Variants} from "framer-motion";
...
const HeroSection = () => {
  ...
  return (
    <section 
      ref={ref}
      className="relative w-full overflow-hidden bg-gray-950 pt-16 md:pt-0 md:h-screen"
    >
      {/* Background Image with Overlay */}
      <div 
        className="absolute inset-0 z-0"
      ...
      {/* Content */}
      <motion.div 
        className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center py-16 md:py-0"
        initial="hidden"
        animate={controls}
        variants={containerVariants}
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center w-full">
          ...
          <div className="text-center lg:text-left">
            <div>
              ...
              >
                <div className="h-0.5 w-8 bg-yellow-400 mr-3"></div>
                <span className="text-yellow-400 font-mono font-medium tracking-wider text-xs sm:text-sm">
                  INDUSTRY LEADERS SINCE 2003
                </span>
              </motion.div>
              ...
              <motion.p 
                className="text-base md:text-lg text-gray-400 mb-6 md:mb-8 max-w-2xl mx-auto lg:mx-0"
                variants={itemVariants}
              >
                We combine cutting edge technology with decades of construction expertise to deliver exceptional results for every project.
              </motion.p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 max-w-xl mx-auto lg:mx-0">
                {stats.map((stat, index) => {
                  ...
                  return (
                    <motion.div 
                      key={index}
                      className="stat-item bg-gray-900/50 p-3 sm:p-4 rounded-lg border border-gray-800 backdrop-blur-sm transition-all duration-300 hover:border-yellow-400/50 hover:scale-105 opacity-0 translate-y-10"
                      initial={{ opacity: 0, y: 20 }}
                    >
                      <div className="flex items-center justify-center lg:justify-start mb-1 sm:mb-2">
                        <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 mr-2" />
          ...
          {/* Right Column - Floating Consultation Card */}
          <motion.div 
            className="relative mt-8 lg:mt-0 lg:block"
            initial="hidden"
            animate={["visible", "float"]}
            variants={floatVariants}
          >
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 shadow-xl">
